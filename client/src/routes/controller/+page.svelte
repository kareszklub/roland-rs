<script lang="ts">
	...
	import { handle_gamepad_connect, handle_gamepad_disconnect } from './gamepad.svelte';
	import { axis_map } from './gamepad.svelte';
...
<svelte:window
	...
	on:gamepadconnected={handle_gamepad_connect}
	on:gamepaddisconnected={handle_gamepad_disconnect}
/>

<Tabs.Root value="wasd">
	<Tabs.List>
		<Tabs.Trigger value="wasd"><KeyboardIcon /> WASD</Tabs.Trigger>
		<Tabs.Trigger value="gamepad"><JoystickIcon /> Gamepad</Tabs.Trigger>
	</Tabs.List>
	<Tabs.Content value="wasd">
	...
	<Tabs.Content value="gamepad">
		<div class="flex space-x-4">
			<Slider
				...
				bind:value={axis_map.ly}
				min={1}
				max={-1}
				onValueChange={() => {
					ws_send_command({ Motor: [-axis_map.ly, -axis_map.ry] });
				}}
			/>
			<Slider
				type="single"
				orientation="vertical"
				bind:value={axis_map.ry}
				min={1}
				max={-1}
				onValueChange={() => {
					ws_send_command({ Motor: [-axis_map.ly, -axis_map.ry] });
